...
from smaclike.mamp.configs.config import eps, DT
from smaclike.mamp.util import distance, absSq, sqr, normalize, l3norm, satisfied_constraint, reached, cartesian2spherical
        ...
        if distance(agent.vel_global_frame, [0.0, 0.0, 0.0]) <= 1e-5:  # the first step is required for dynamic
            ...
            for obj in agent.neighbors:
                ...
                if distSq > combinedRadiusSq:  # No collision.
                    ...
                    # print('Collision.')
                # if obstacle is static
                if obj.is_obstacle : 
                    plane.point = agent.vel_global_frame + u
