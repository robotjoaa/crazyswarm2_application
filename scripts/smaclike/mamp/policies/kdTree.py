...
from smaclike.mamp.util import l2norm, leftOf, det, l2normsq, sqr
...
class KDTree(object):
    def __init__(self, agents, obstacles):
        ...
        self.agentIDs = []
        # obj.id to agent id mapping
        self.reverse_map = {obj.id: i for i, obj in enumerate(agents)}
        for obj in agents:
    ...
    def updateAgentTree(self, new_agents):
        self.agents = new_agents
        self.agentTree = []        
        for i in range(2 * len(new_agents) - 1):
            self.agentTree.append(AgentTreeNode())
        self.agentIDs = []
        self.reverse_map = {obj.id: i for i, obj in enumerate(new_agents)}
        for obj in new_agents:
            #self.agentIDs.append(obj.id)
            self.agentIDs.append(self.reverse_map[obj.id])
    ...
    def computeAgentNeighbors(self, agent, rangeSq, option=None):
        self.queryAgentTreeRecursive(agent, rangeSq, 0, option)

    def queryAgentTreeRecursive(self, agent, rangeSq, node, option=None):
        if self.agentTree[node].end - self.agentTree[node].begin <= self.max_leaf_size:
            for i in range(self.agentTree[node].begin, self.agentTree[node].end):
                agent.insertAgentNeighbor(self.agents[self.agentIDs[i]], rangeSq, option)
        else:
            distSqLeft = (
    ...
    def buildObstacleTreeRecursive(self, begin, end, node):
        ...
        # 不直接用数组进行   赋值是因为会指向同一地址，智能体的当前位置也会跟随minCoord, maxCoord改变
